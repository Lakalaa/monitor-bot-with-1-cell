...
async def apply_stealth_privacy(client: TelegramClient, session_num: int):
    ...
    logger.info('Session #%d — stealth privacy applied', session_num)
...
async def main():
    # Wait until sessions are configured (Render redeploys when TG_SESSIONS is added)
    sessions = get_sessions()
    if not sessions:
        logger.info('No TG_SESSIONS configured — userbot standing by, will retry every 30s...')
        while not sessions:
            await asyncio.sleep(30)
            sessions = get_sessions()
        logger.info('Sessions found! Starting %d session(s)...', len(sessions))

    if not API_ID or not API_HASH:
        logger.warning('TG_API_ID / TG_API_HASH missing — userbot disabled')
        while True:
